...
from working_dir_root import Thoracic_select,Video_down_sample_f,Dataset_video_pkl_thoracic_test,Dataset_video_pkl_pascal,Dataset_video_pkl_coco,Dataset_video_pkl_poem
# from working_dir_root import Dataset_video_pkl_MOVIE,Dataset_video_pkl_MOVID
...
PASCAL_data = "PASCAL" in selected_data
COCO_data = "COCO" in selected_data
POEM_data = "POEM" in selected_data
MOVIE_data = "MOVIE" in selected_data
MOVID_data = "MOVID" in selected_data
...
# Validation logic
flags= [Cholec_data_flag,Thoracic_data_flag,MICCAI_data,MICCAI_data_merge,MICCAIv2,PASCAL_data,
        Endovis_data,DAVIS_data, YTVOS_data,YTOBJ_data,COCO_data,POEM_data,MOVID_data,MOVIE_data]
if not any(flags):
...
#     ''
# ]
categories=['mucosa', 'muscle', 'ttknife']
# category_colors = {
...
class myDataloader(object):
    def __init__(self, OLG=False,img_size = 128,Display_loading_video = False,
                 Read_from_pkl= True,Save_pkl = False,Load_flow =False,Load_feature=True, Train_list='else',
                 starting_percentage=0.0,Device=device, gt_mask_dir=None):
        ...
        if self.gt_mask_dir:
            if not os.path.isdir(self.gt_mask_dir):
                print(f"Warning: GT mask directory {self.gt_mask_dir} does not exist")
                self.gt_mask_files = {}
            else:
                # Create mapping of video name to mask path (without extension)
                self.gt_mask_files = {
                    os.path.splitext(f)[0]: os.path.join(self.gt_mask_dir, f) 
                    for f in os.listdir(self.gt_mask_dir) 
                    if f.endswith('.pkl')
                }
        else:
            self.gt_mask_files = {}
        ...
        if Read_from_pkl == False:
            ...
            if Cholec_data_flag == True:
                # all files
                all_files = os.listdir(Dataset_video_pkl_cholec)
                if Train_list == "train":
                    # train_set_no8k
                    ...
                    files= io.read_a_pkl(train_test_list_dir, 'train_set_balance_no8K')
                elif Train_list == "train_raw":
                    files = io.read_a_pkl(train_test_list_dir, 'train_set_no8k')
                elif Train_list == "small":
                    files = io.read_a_pkl(train_test_list_dir, 'train_set')
                elif Train_list == "train_set_partition":
                    files = io.read_a_pkl(train_test_list_dir, 'train_set_partition')
                elif Train_list == "test":
                    train_files = io.read_a_pkl(train_test_list_dir, 'train_set_no8k')
                    test_files = list(set(all_files) - set(train_files))
                    files = test_files
                elif Train_list == "else":
                    files = all_files
                if Test_on_cholec_seg8k ==True:
            ...
            if YTVOS_data:
                files = os.listdir (Dataset_video_pkl_YTVOS)
                self.all_video_dir_list.extend([(f, 'ytvos') for f in files])
            if YTOBJ_data:
                files = os.listdir (Dataset_video_pkl_YTOBJ)
                self.all_video_dir_list.extend([(f, 'ytobj') for f in files])
                # self.all_video_dir_list = os.listdir (Dataset_video_pkl_YTOBJ)
            if PASCAL_data:
                files = os.listdir (Dataset_video_pkl_pascal)
                self.all_video_dir_list.extend([(f, 'pascal') for f in files])
            if POEM_data:   
                files = os.listdir (Dataset_video_pkl_poem)
                self.all_video_dir_list.extend([(f, 'poem') for f in files])
            if COCO_data:
                files = os.listdir (Dataset_video_pkl_coco)
                self.all_video_dir_list.extend([(f, 'coco') for f in files])
            if MOVIE_data:
                files = os.listdir (Dataset_video_pkl_MOVIE)
                self.all_video_dir_list.extend([(f, 'movie') for f in files])
            if MOVID_data:
                files = os.listdir (Dataset_video_pkl_MOVID)
                self.all_video_dir_list.extend([(f, 'movid') for f in files])
        ...
        if Evaluation_slots == False:
            # Calculate subset indices using both percentages
            total_videos = len(self.all_video_dir_list)
            start_idx = int(total_videos * self.starting_percentage)
            end_idx = int(total_videos * (self.starting_percentage + self.dataset_percentage))

            # Ensure we don't exceed list bounds
            end_idx = min(end_idx, total_videos)
            self.all_video_dir_list = self.all_video_dir_list[start_idx:end_idx]
    ...
    def _get_folder_path(self, dataset_source):
        """Get the appropriate folder path for a dataset source"""
        path_mapping = {
            'cholec': Dataset_video_pkl_cholec,
            'thoracic': Dataset_video_pkl_thoracic,
            'miccai': Dataset_video_pkl_root,
            'endovis': Dataset_video_pkl_endovis,
            'ytvos': Dataset_video_pkl_YTVOS,
            'ytobj': Dataset_video_pkl_YTOBJ,
            'pascal': Dataset_video_pkl_pascal,
            'poem': Dataset_video_pkl_poem,
            'coco': Dataset_video_pkl_coco,
            'movie': Dataset_video_pkl_MOVIE,
            'movid': Dataset_video_pkl_MOVID,
            'miccai_v2': Dataset_video_pkl_MICCAIv2,
            'miccai_merge': Dataset_video_pkl_merge_root
        }
        return path_mapping.get(dataset_source, Dataset_video_pkl_root)
    ...
    def read_a_batch(self, this_epoch=0): # use this epoch to slide the loader window in data is longer than that
        ...
        self.batch_files = []  # Initialize list to track filenames and sources
        self.all_raw_labels=[]
        ...
        while(1): # load a batch of images
            ...
            self.batch_files.append((filename, dataset_source))  # Track the file info
            ...
            if dataset_source == 'cholec':
                ...
                folder_path = Dataset_video_pkl_endovis
            elif  dataset_source == 'ytvos':
                folder_path = Dataset_video_pkl_YTVOS
            elif  dataset_source == 'ytobj':
                folder_path = Dataset_video_pkl_YTOBJ
            elif  dataset_source == 'pascal':
                folder_path = Dataset_video_pkl_pascal
            elif  dataset_source == 'poem':
                folder_path = Dataset_video_pkl_poem
            elif  dataset_source == 'coco':
                folder_path = Dataset_video_pkl_coco
            elif  dataset_source == 'movie':
                folder_path = Dataset_video_pkl_MOVIE
            elif  dataset_source == 'movid':
                folder_path = Dataset_video_pkl_MOVID
            ...
            for i in range(self.batch_size): # load a batch of images
                ...
                if filename.endswith(file_name_extention):
                    ...
                    if clip_name in self.gt_mask_files:
                        try:
                            # Use the read_a_pkl function to load
                            mask_data = io.read_a_pkl(self.gt_mask_dir, clip_name)
                            
                            if mask_data is not None:
                                masks=mask_data['masks']  # Assuming the pkl file directly contains the mask array
                                ...
                                if masks is not None:
                                    print(f"Loaded mask for {clip_name}, shape: {masks.shape}")
                                    # Ensure masks are in [C, D, H, W] format
                                    if masks.ndim == 3:  # [D, H, W] - single channel
                                        masks = np.expand_dims(masks, 0)  # Add channel dim [1, D, H, W]
                                        masks = np.repeat(masks, self.obj_num, axis=0)  # Repeat for all tools [C, D, H, W]
                                    elif masks.ndim == 4:  # [C, D, H, W]
                                        if masks.shape[0] != self.obj_num:
                                            print(f"Warning: Mask has {masks.shape[0]} channels, expected {self.obj_num}")
                                            # If mask has different number of channels, adapt it
                                            if masks.shape[0] < self.obj_num:
                                                # Pad with zeros
                                                padded_masks = np.zeros((self.obj_num, masks.shape[1], masks.shape[2], masks.shape[3]))
                                                padded_masks[:masks.shape[0]] = masks
                                                masks = padded_masks
                                            else:
                                                # Truncate to obj_num
                                                masks = masks[:self.obj_num]
                                    else:
                                        print(f"Warning: Invalid mask dimensions in {clip_name}: {masks.ndim}")
                                        masks = None
                                    
                                    if masks is not None:
                                        # Trim to video length and resize
                                        masks = masks[:, :self.video_len]
                                        if masks.shape[-2:] != (self.image_size, self.image_size):
                                            resized_masks = np.zeros((masks.shape[0], masks.shape[1], 
                                                                self.image_size, self.image_size))
                                            for c in range(masks.shape[0]):
                                                for d in range(masks.shape[1]):
                                                    resized_masks[c,d] = cv2.resize(
                                                        masks[c,d], 
                                                        (self.image_size, self.image_size), 
                                        ...
                                        # Initialize batch_masks if not done yet
                                        if batch_masks is None:
                                            batch_masks = np.zeros((self.batch_size, self.obj_num, self.video_len, 
                                                                self.image_size, self.image_size))
                            ...
                            print(f"Error processing mask for {clip_name}: {str(e)}")
                            self.gt_masks = None
            ...
            if filename.endswith(file_name_extention):
                ...
                if self.Read_from_pkl == False:
                    ...
                    if DAVIS_data == True:                     
                        ...
                        self.video_buff = this_video_buff 
                    if dataset_source == "ytvos":                     
                        data_dict = io.read_a_pkl(Dataset_video_pkl_YTVOS, clip_name)
                        this_video_buff = data_dict['frames']
                        
                        labels= data_dict['labels']      # (7,29) for cholec , (13, 29, 256, 256) for seg8k
                        # labels= data_dict['super_labels']      # (7,29) for cholec , (13, 29, 256, 256) for seg8k
                        self.this_masks = data_dict['masks'] 

                        self.this_frame_label = labels
                        
                        self.video_buff = this_video_buff
                        Valid_video_flag = True
                    if dataset_source == "movid":                     
                        data_dict = io.read_a_pkl(Dataset_video_pkl_MOVID, clip_name)
                        this_video_buff =  data_dict['frames'] 
                        
                        labels=  data_dict['masks']    #  3,1,224,224
                        # self.this_frame_label = labels
                        
                        self.video_buff = this_video_buff 
                        Valid_video_flag = True
                    if dataset_source == "movie":                     
                        data_dict = io.read_a_pkl(Dataset_video_pkl_MOVIE, clip_name)
                        this_video_buff =  data_dict['frames']
                    ...
                    if dataset_source == 'thoracic':
                        ...
                        self.video_buff = this_video_buff 
                        Valid_video_flag = True
                    if dataset_source == 'pascal':
                        data_dict = io.read_a_pkl(Dataset_video_pkl_pascal, clip_name)
                        this_video_buff =  data_dict['image'] 
                        
                        labels=  data_dict['mask']    #  3,1,224,224
                        # self.this_frame_label = labels
                        
                        self.video_buff = this_video_buff 
                        Valid_video_flag = True
                    # if dataset_source == 'poem':
                    #     data_dict = io.read_a_pkl(Dataset_video_pkl_poem, clip_name)
                    #     this_video_buff =  data_dict['image'] 
                    ...
                    if dataset_source == 'poem':
                        ...
                        this_video_buff = data_dict['frames']               # [3, 8, H, W] or [C, T, H, W]
                        masks = data_dict['masks']                           # [3, 8, H, W] – ground truth masks
                        # Optionally you could also read data_dict['labels'] if it contains per‑frame presence,
                        # but we can compute presence from masks as before.
                        labels = masks                                        # keep variable name for compatibility
                        self.video_buff = this_video_buff
                        Valid_video_flag = True
                    if dataset_source == 'coco':
                        data_dict = io.read_a_pkl(Dataset_video_pkl_coco, clip_name)
                        this_video_buff =  data_dict['image'] 
                ...
                if ( (dataset_source == 'miccai' and (clip_name in self.all_labels)) or Valid_video_flag==True):  
                    ...
                    if MICCAIv2 == True:
                        ...
                        binary_vector_l = 0
                        binary_vector_r = 0
                    # FOR TRAINING UNCOMMENT OR MINORITY CLASS TESTING
                    # if dataset_source == 'cholec':
                    #     binary_vector = self.merge_labels(labels)
                    #     # binary_vector = np.ones(len(categories))
                    #     self.this_video_label = binary_vector
                    #     if Test_on_cholec_seg8k:
                    #         # mask,frame_label,video_label = format_convertor.label_from_seg8k_full(labels)
                    #         mask,frame_label,video_label = format_convertor.label_from_full_cholec(labels)
                    #         # # 
                    #         # if len(clip_name)<13:
                    ...
                    if dataset_source == 'cholec':                     
                        ...
                        masks = data_dict['masks']      # (7, 29, H, W) - the actual mask arrays
                        labels = data_dict['labels']     # {'Grasper': 0/1, ...} - the tool presence dict
                        
                        self.this_frame_label = labels   # Keep this for reference
                        
                        self.video_buff = this_video_buff 
                        binary_vector = self.merge_labels(labels)  # This will fail - labels is a dict
                        # binary_vector = np.ones(len(categories))
                        self.this_video_label = binary_vector
                        if Test_on_cholec_seg8k:
                            mask, frame_label, video_label = format_convertor.label_from_full_cholec(masks)
                        ...
                        binary_vector_l = 0
                        binary_vector_r = 0
                    if dataset_source == "movid" or dataset_source == "movie":
                        # binary_vector = self.merge_labels(labels)
                        # binary_vector = binary_vector[0:len(categories)]
                        binary_vector= np.ones(25)

                        if Evaluation_slots == True:
                             
                            mask = format_convertor.color_mask_to_instance_mask_ytvos(labels,max_instances=25)
                            self.this_label_mask = mask 
                        self.this_video_label = binary_vector
                        self.this_frame_label = labels
                    ...
                    if dataset_source == 'ytobj':
                        ...
                        binary_vector = self.merge_labels(labels)
                        mask = format_convertor.label_from_ytobj(self.this_masks, binary_vector)
                        # for slot onlu
                        binary_vector= np.ones(len(categories))
                        self.this_video_label = binary_vector
                        # self.this_video_label = binary_vector
                    ...
                    if dataset_source == 'thoracic' :
                        if  Evaluation_slots == True: 
                            mask,frame_label,video_label = format_convertor.label_from_thoracic_full(labels)
                    ...
                    if dataset_source == 'pascal' :
                        if  Evaluation_slots == True: 
                            ...
                            mask,frame_label,video_label = format_convertor.label_from_pascal(labels)
                        else:
                            mask=None
                            frame_label =0
                            video_label= np.ones(len(categories))
                    ...
                    #     binary_vector_r = 0
                    if dataset_source == 'poem':
                        # labels        : [3, 8, H, W]  float32  mucosa / muscle / ttknife
                        # binary_vector : [3]            float32  1.0 if class present in any frame
                        binary_vector         = (labels.reshape(labels.shape[0], -1)
                                                 .max(axis=1) > 0).astype(np.float32)  # [3]
                        self.this_video_label = binary_vector
                        self.this_frame_label = labels                   # [3, 8, H, W]
                        self.this_label_mask  = labels
                        self.this_raw_labels  = labels
                        binary_vector_l = 0
                        binary_vector_r = 0
                        # --- Load GT masks directly from the pkl (no separate gt_mask_dir needed) ---
                        try:
                            poem_masks = labels.copy()                   # [3, 8, H, W]
                            n_frames   = poem_masks.shape[1]             # 8
                            # Resize spatial dims to model input size if needed
                            if poem_masks.shape[-2:] != (self.image_size, self.image_size):
                                resized = np.zeros((poem_masks.shape[0], n_frames,
                                                    self.image_size, self.image_size),
                                                   dtype=np.float32)
                                for c in range(poem_masks.shape[0]):
                                    for d in range(n_frames):
                                        resized[c, d] = cv2.resize(
                                            poem_masks[c, d],
                                            (self.image_size, self.image_size),
                                            interpolation=cv2.INTER_NEAREST)
                                poem_masks = resized
                            self.gt_masks  = poem_masks                  # [3, 8, img_size, img_size]
                            mask_available = True
                            if batch_masks is None:
                                batch_masks = np.zeros((self.batch_size, self.obj_num,
                                                        n_frames,
                                                        self.image_size, self.image_size),
                                                       dtype=np.float32)
                            batch_masks[i] = poem_masks 
                        except Exception as _e:
                            print(f"[POEM] GT mask error for {clip_name}: {_e}")
                    if dataset_source == 'coco' :
                        if  Evaluation_slots == True and labels is not None: 

                            mask,frame_label,video_label = format_convertor.label_from_coco(labels)
                        else:
                            mask=None
                            frame_label =0
                            video_label= np.ones(len(categories))
                    ...
                    if dataset_source == 'cholec' or dataset_source == 'thoracic' or dataset_source == 'endovis' or dataset_source == 'ytvos' or  dataset_source == 'ytobj' or dataset_source=='poem':
                    ...
                    if Crop_half==True  and Crop_flag==True:
                        self.video_buff=basic_operator.random_crop_and_flip(self.video_buff) # for slot attention
        ...
        if Seperate_LR == False:
            if mask_available:
                return self.input_videos, self.labels, batch_masks
            else:
                return self.input_videos, self.labels, None
        else:
            if mask_available:
                return self.input_videos, self.labels_LR, batch_masks
            else:
                return self.input_videos, self.labels_LR, None
            
    # def refresh_ground_truth_list(self):
    #     """Refresh the list of ground truth files"""
    #     if hasattr(self, 'gt_mask_dir') and self.gt_mask_dir:
    #         self.gt_mask_files = {
    #             os.path.splitext(f)[0]: os.path.join(self.gt_mask_dir, f) 
    #             for f in os.listdir(self.gt_mask_dir) 
    #             if f.endswith('.pkl')
    #         }
    #         print(f"Refreshed ground truth list: {len(self.gt_mask_files)} files available")
    def refresh_ground_truth_list(self):
        """Refresh the list of ground truth mask files"""
        if self.gt_mask_dir:
            if not os.path.isdir(self.gt_mask_dir):
                print(f"Warning: GT mask directory {self.gt_mask_dir} does not exist")
                self.gt_mask_files = {}
                ...
                print(f"Refreshed ground truth mask list: {len(self.gt_mask_files)} files available")
        else:
            self.gt_mask_files = {}
            print("No ground truth mask directory specified")
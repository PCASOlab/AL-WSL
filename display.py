...
from dataset.dataset import myDataloader,categories,category_colors, Endovis_data
...
class Display(object):
    ...
    def train_display(self,MODEL_infer,mydata_loader, read_id,Output_root):
        ...
        if torch.is_tensor(MODEL_infer.final_output):
            self.Model_infer.output = MODEL_infer.final_output.detach()
        else:
            print("Warning: Unexpected output type")
            return

        # Handle other attributes
        self.Model_infer.cam3D = MODEL_infer.cam3D.detach() if hasattr(MODEL_infer, 'cam3D') else None
        self.Model_infer.raw_cam = MODEL_infer.raw_cam.detach() if hasattr(MODEL_infer, 'raw_cam') else None
        ...
        if hasattr(MODEL_infer, 'sam_mask') and MODEL_infer.sam_mask is not None:
            if torch.is_tensor(MODEL_infer.sam_mask):
                if not torch.isnan(MODEL_infer.sam_mask).any():
                    self.Model_infer.sam_mask = MODEL_infer.sam_mask.detach().cpu()
                else:
                    print("Warning: SAM mask contains NaN values")
                    self.Model_infer.sam_mask = torch.zeros_like(MODEL_infer.cam3D).detach().cpu()
            else:
                print("Warning: SAM mask is not a tensor")
                self.Model_infer.sam_mask = torch.zeros_like(MODEL_infer.cam3D).detach().cpu()
        else:
            print("SAM mask not available - using zero mask")
            self.Model_infer.sam_mask = torch.zeros_like(MODEL_infer.cam3D).detach().cpu()
    ...
    def visualize_masks(self, model_output, data_loader, iteration, output_dir):
        """
        Visualize predicted masks with a clean interface that matches your existing code
        
        Args:
            model_output: Your Model_infer object containing predictions
            data_loader: Your data loader instance
            iteration: Current iteration number
            output_dir: Directory to save visualizations
        """
        # Get the first sample in batch
        input_video = data_loader.input_videos[0]  # [C,T,H,W] or [C,H,W]
        predicted_masks = model_output.cam3D[0]    # [1,T,H,W] or [T,H,W]
        
        # Convert to numpy
        if isinstance(input_video, torch.Tensor):
            input_video = input_video.detach().cpu().numpy()
        if isinstance(predicted_masks, torch.Tensor):
            predicted_masks = predicted_masks.detach().cpu().numpy()
        
        # Handle different input dimensions
        if input_video.ndim == 3:  # [C,H,W] -> add temporal dim
            input_video = input_video[:, np.newaxis, :, :]  # [C,1,H,W]
    
        # Upscale masks from 28x28 to 128x128
        upscaled_masks = np.zeros((predicted_masks.shape[0], predicted_masks.shape[1], 128, 128))
        for c in range(predicted_masks.shape[0]):  # For each class
            for t in range(predicted_masks.shape[1]):  # For each frame
                upscaled_masks[c,t] = cv2.resize(
                    predicted_masks[c,t], 
                    (128, 128), 
        ...
        for c in range(upscaled_masks.shape[0]):
            class_dir = os.path.join(output_dir, f"class_{c}")
            os.makedirs(class_dir, exist_ok=True)
            
            for t in range(min(input_video.shape[1], upscaled_masks.shape[1])):
                # Get frame [C,H,W] -> [H,W,C]
                frame = np.transpose(input_video[:, t], (1, 2, 0))
                frame = (frame * 255).astype(np.uint8)
                if frame.shape[-1] == 1:  # Handle grayscale
                    frame = np.repeat(frame, 3, axis=-1)
                
                # Get mask [H,W]
                mask = upscaled_masks[c,t]
                ...
                # Save visualization
                output_path = os.path.join(class_dir, f"frame_{t:03d}.png")
                cv2.imwrite(output_path, cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR))
        
        print(f"Saved mask visualizations to {output_dir}")
def stack_to_color_mask (final_mask,Ori_H,Ori_W,output_0,label_0,step_l):
    ...
    for j in range(len(final_mask)):
            ...
            color_mask[this_mask_channel > 0.3] = list(category_colors.values())[j%len(category_colors)]
            if (output_0[j]>0.5 or label_0[j]>0.5):
                for i in range(0, D, step_l):

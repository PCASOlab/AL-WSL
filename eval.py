...
def _mask_to_box_mask(mask2d: torch.Tensor) -> torch.Tensor:
    """
    Convert a 2D binary/soft mask to a filled box mask covering its tight bbox.
    Returns all-zeros if the mask is empty.
    """
    # Treat any positive value as foreground; adjust threshold if needed
    ys, xs = (mask2d > 0).nonzero(as_tuple=True)
    out = torch.zeros_like(mask2d)
    ...
    out[y0:y1+1, x0:x1+1] = 1 if mask2d.dtype in (torch.uint8, torch.int8, torch.int16, torch.int32, torch.int64) else mask2d.new_tensor(1.0)
    return out

def cal_corloc_box(true, predict, iou_threshold=0.2):
    """
    CorLoC using box-masks: each (frame, class) mask is converted to its tight
    bounding box (filled), then IoU is computed between the two boxes.
    
    Args:
        true:     Ground truth mask tensor of shape (ch, D, H, W)
        predict:  Predicted mask tensor of shape (ch, D, H, W)
        iou_threshold: threshold for IoU to count as correct
    
    Returns:
        corloc_score (float), corloc_per_frame (list of floats)
    """
    assert true.shape == predict.shape, f"Shape mismatch: {true.shape} vs {predict.shape}"
    ch, D, H, W = true.shape
    corloc_per_frame = []
    ...
    for frame_idx in range(D):
        ...
        for class_idx in range(ch):
            true_frame = true[class_idx, frame_idx]
            pred_frame = predict[class_idx, frame_idx]

            # Use original GT mask to decide validity (same as your code)
            if torch.sum(true_frame) > 10:
                valid_classes += 1

                # Convert to box masks
                true_box_mask = _mask_to_box_mask(true_frame)
                pred_box_mask = _mask_to_box_mask(pred_frame)

                # Your existing IoU function over binary masks
                iou = cal_J_safe(true_box_mask, pred_box_mask)

                if not torch.isnan(iou) and iou > iou_threshold:
                    frame_corloc += 1

        frame_corloc_score = (frame_corloc / valid_classes) if valid_classes > 0 else float('nan')
        corloc_per_frame.append(frame_corloc_score)

    valid_frames = [s for s in corloc_per_frame if not np.isnan(s)]
    corloc_score = float(np.mean(valid_frames)) if len(valid_frames) > 0 else float('nan')
    return corloc_score, corloc_per_frame
...
def cal_tnr(true, predict, class_indices):
    ...
    return tnr
def cal_J_safe(true, predict):
    """Safe IoU calculation that returns NaN when both masks are empty"""
    AnB = true * predict
    AuB = true + predict
    AuB = torch.clamp(AuB, 0, 1)
    
    intersection = torch.sum(AnB)
    union = torch.sum(AuB)
    
    # If both masks are empty, return NaN tensor
    if union == 0:
        return torch.tensor(float('nan'), device=true.device)
    
    return intersection / union

def cal_dice_safe(true, predict):
    """Safe Dice calculation that returns NaN when both masks are empty"""
    intersection = torch.sum(true * predict)
    union = torch.sum(true) + torch.sum(predict)
...
def cal_all_metrics(read_id, Output_root, label_mask, frame_label, video_label,
    ...
    if label_mask.shape[0] > output_video_label.shape[0]:
        label_mask  = label_mask[:-1]          # (ch-1, D, H, W)
        video_label = video_label[:-1]         # (ch-1,)
        frame_label = frame_label[:, :-1]      # (D, ch-1)
    ch, D, H, W = label_mask.size()
    while predic_mask_3D.dim() > 4:
        predic_mask_3D = predic_mask_3D.squeeze(0)
    ...
    if frame_label.dim() == 4:
    # POEM sends (ch, D, H, W) spatial masks — collapse to (ch, D) presence flags
        frame_label = (frame_label.amax(dim=(-1, -2)) > 0).float()
    else:
    ...
    # Global IoU / Dice (all classes together) - keep .item() since these return tensors
    IoU = round(cal_J_safe(label_mask[0], predic_mask_3D[0]).item(), 4)
    print("Intersection over Union (IoU):", IoU)

    dice = round(cal_dice_safe(label_mask, predic_mask_3D).item(), 4)
    print("Dice coefficient:", dice)

    # NEW: Calculate CorLoC metric
    corloc_overall, corloc_per_frame = cal_corloc_box(label_mask, predic_mask_3D)
    print("CorLoC (Correct Localization):", round(corloc_overall, 4) if not np.isnan(corloc_overall) else "NaN")
    ...
    IoU_maskout = round(cal_J_safe(label_mask * predic_frame_expanded, predic_mask_3D * predic_frame_expanded).item(), 4)
    print("IoU for positive frames only:", IoU_maskout)

    Dice_maskout = round(cal_dice_safe(label_mask * predic_frame_expanded, predic_mask_3D * predic_frame_expanded).item(), 4)
    ...
    metrics_video_data.append({
        ...
        'CorLoC': corloc_overall if not np.isnan(corloc_overall) else float('nan')  # NEW
    })
    ...
    # NEW: Add CorLoC per frame
    for i, corloc_frame in enumerate(corloc_per_frame):
        new_frame_data[f'Frame_{i+1}_CorLoC'] = corloc_frame if not np.isnan(corloc_frame) else float('nan')
    metrics_frame_data.append(new_frame_data)

    # =========================
    # NEW: Per-class metrics with safe handling
    # =========================
    ...
    for c in range(ch):
        ...
        # Per-class IoU/Dice over all frames & pixels - with safe handling
        # Use try-except to handle NaN values safely
        try:
            iou_c = cal_J_safe(label_mask[c], predic_mask_3D[c]).item()
            iou_c = round(iou_c, 4) if not torch.isnan(torch.tensor(iou_c)) else float('nan')
        except:
            iou_c = float('nan')
            
        try:
            dice_c = cal_dice_safe(label_mask[c], predic_mask_3D[c]).item()
            dice_c = round(dice_c, 4) if not torch.isnan(torch.tensor(dice_c)) else float('nan')
        except:
            dice_c = float('nan')

        # NEW: Per-class CorLoC
        corloc_c, _ = cal_corloc_box(label_mask[c].unsqueeze(0), predic_mask_3D[c].unsqueeze(0))
        corloc_c = round(corloc_c, 4) if not np.isnan(corloc_c) else float('nan')

        # Masked by positive frames (for that class only)
        pf_exp_c = predic_frame_bin[c].reshape(D, 1, 1).repeat(1, H, W)
        
        try:
            iou_pos_c = cal_J_safe(label_mask[c] * pf_exp_c, predic_mask_3D[c] * pf_exp_c).item()
            iou_pos_c = round(iou_pos_c, 4) if not torch.isnan(torch.tensor(iou_pos_c)) else float('nan')
        except:
            iou_pos_c = float('nan')
            
        try:
            dice_pos_c = cal_dice_safe(label_mask[c] * pf_exp_c, predic_mask_3D[c] * pf_exp_c).item()
            dice_pos_c = round(dice_pos_c, 4) if not torch.isnan(torch.tensor(dice_pos_c)) else float('nan')
        except:
            dice_pos_c = float('nan')
        ...
        metrics_video_class_data.append({
            ...
            'IoU': iou_c,
            'IoU_Positive_Frames': iou_pos_c,
            'Dice_Coefficient': dice_c,
            'Dice_Coefficient_Positive_Frames': dice_pos_c,
            'CorLoC': corloc_c  # NEW
        })
    ...
    # =========================
    # Save all CSVs with NaN-aware averaging
    # =========================
    ...
    # Existing averages (skip NaN values)
    ...
    if len(metrics_frame_data) > 0:
        ...
        all_frame_values = all_frame_values[~np.isnan(all_frame_values)]
    ...
    # ---- NEW: save per-class CSVs + per-class averages with NaN handling ----
    if len(metrics_video_class_data) > 0:
        ...
        # averages per class across videos (pandas groupby mean automatically skips NaN by default)

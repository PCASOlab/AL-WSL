...
import torch.utils.data
from torch.autograd import Variable
from model import  model_experiement, model_infer,model_infer_TC_res
...
Output_root = Output_root+ "temporal_consistent/" +selected_data +"res"+'/'
...
Model_infer = model_infer_TC_res._Model_infer(GPU_mode,num_gpus,Enable_teacher=True,gpu_selection="0",pooling="rank",TPC=True)
...
if Continue_flag == False:
    ...
    Model_infer.VideoNets_S.apply(weights_init)
    ...
    # outNets_s
    Model_infer.VideoNets_S.load_state_dict(torch.load(Output_root + 'outNets_s' + loadmodel_index, map_location= "cuda:0") )

    Model_infer.VideoNets.load_state_dict(torch.load(Output_root + 'outNets' + loadmodel_index, map_location= "cuda:0") )
    Model_infer.resnet.load_state_dict(torch.load(Output_root + 'outResNets' + loadmodel_index, map_location= "cuda:0") )

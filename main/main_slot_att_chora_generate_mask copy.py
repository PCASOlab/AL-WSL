...
from working_dir_root import Enable_student,Batch_size,selected_data,slot_out_dir,Display_down_sample
from dataset import io
import pathlib
import argparse

Slot_attention_save_result = True
# GPU_mode= True
# Continue_flag = True
# Visdom_flag = False
# Display_flag = False
...
Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),GPU_mode,num_gpus,Enable_teacher=True,gpu_selection="1",pooling="max",TPC=True)
...
if Continue_flag == False:
    ...
    Model_infer.model.initializer.load_state_dict(torch.load(Output_root + 'initializer' + loadmodel_index ))
    Model_infer.model.encoder.load_state_dict(torch.load(Output_root + 'encoder' + loadmodel_index ))
    Model_infer.model.processor.load_state_dict(torch.load(Output_root + 'processor' + loadmodel_index ))
    Model_infer.model.decoder.load_state_dict(torch.load(Output_root + 'decoder' + loadmodel_index ))
...
while (1):
    ...
    if Slot_attention_save_result == True:
        this_input = Model_infer.input_resample

        this_mask= Model_infer.cam3D
        this_features = Model_infer.output["encoder"]["features"].half()
        this_slots = Model_infer.output["processor"]["state"].half()
    

        data_dict = {'input': this_input,
                    'feature': this_features,
                     'slots': this_slots,
                     'mask': this_mask}
        slot_pkl_file_name = dataLoader.this_file_name
        slot_pkl_file_path = os.path.join(slot_out_dir, slot_pkl_file_name)

        with open(slot_pkl_file_path, 'wb') as file:
            pickle.dump(data_dict, file)
            print("sam Pkl file created:" + slot_pkl_file_name)
    ...
    if (read_id % 1000) == 0  :
        torch.save(Model_infer.model.state_dict(), Output_root + "outNets" + str(saver_id) + ".pth")

...
from working_dir_root import Enable_student,Batch_size,selected_data,sam_feature_OLG_dir2, Display_down_sample
...
dataLoader = myDataloader(img_size = 128,Display_loading_video = False,Read_from_pkl= True,Save_pkl = False,Load_flow=Load_flow, Load_feature=Load_feature,Train_list='else',Device=device)
...
if Continue_flag == False:
    ...
    Model_infer.backbone.load_state_dict( torch.load(Output_root + 'outNets_vit' + loadmodel_index ) )
    pass
...
while (1):
    start_time = time()
    input_videos, labels,_= dataLoader.read_a_batch()
    ...
    if Save_batch2mp4 == True:
        ...
        if any_hit:
            ...
            try:
                ...
                base_name = os.path.splitext(base_name)[0]  # strip .pkl if present
    ...
    if (read_id % 1000) == 0  :
        ...
        torch.save(Model_infer.backbone.state_dict(), Output_root + "outNets_vit" + str(saver_id) + ".pth")

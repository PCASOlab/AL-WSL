...
class _VideoCNN(nn.Module):
    ...
    def __init__(self, inputC=512,base_f=1024,pooling="rank"):
        ...
        #
        base_f1= base_f
        self.blocks.append(block_buider.conv_keep_all(inputC, 2048,k=(1,1,1), s=(1,1,1), p=(0,0,0), resnet= False,final=False,dropout = False))
        # self.blocks.append(nn.AvgPool3d((1,2,2),stride=(1,2,2)))
       
        base_f2= base_f//2

        self.blocks.append(block_buider.conv_keep_all(2048, 256,k=(1,1,1), s=(1,1,1), p=(0,0,0),resnet = False,final=True,dropout = F))
        ...
        self.classifier = nn.Conv3d(int( 256), Obj_num , (1,1,1), (1,1,1), (0,0,0), bias=False)  # 4*256
...
# Step 2: Get the indices of the top k values along the flattened dimension
    ...
    def maxpooling(self,input):
        ...
        if flag == True: 
            ...
            # slice_valid = self.Top_rank_pooling_HW(input,percentage=0.5)
            # 
            Maxpool_keepC = nn.MaxPool3d((D,1,1),stride=(1,1,1))
        ...
        if self.pooling == "avg":
            ...
            final = self.Top_rank_pooling(slice_valid,25)

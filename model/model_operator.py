...
def hide_patch(video_feature, patch_num=28*28, hide_prob=0.8, value=0,image_level= False):
    ...
    for (px, py) in patch_offsets:
            if np.random.uniform() < hide_prob:
                    video_feature[:, :, :,px:px + patch_size, py:py + patch_size] = value # only deal train
...
def Cam_mask_post_process(raw_masks,input,video_predict,multimask_output: bool = False):
        ...
        bz, ch, D, H, W = raw_masks.size()
        video_predict = video_predict>0.1
        ...
        for i in range(ch):
            ...
            raw_masks[:,i,:,:,:] = raw_masks[:,i,:,:,:] /(torch.max(raw_masks[:,i,:,:,:] )+0.0000001)*1.5
        # raw_masks = raw_masks /(50+0.0000001) 
        # raw_masks = raw_masks*label_valid_repeat
        winners = raw_masks.argmax(dim=1, keepdim=True)  # [bz, 1, D, H, W]
    
    # Create a mask where only the winning channels keep their values, others are zero
        binary_mask = torch.zeros_like(raw_masks)
        binary_mask.scatter_(1, winners, 1.0)  # Set winning positions to 1
        # processed_masks = raw_masks * binary_mask  # Element-wise multiplication
        # winners = raw_masks.argmax(dim=1, keepdim=False)                  # [bz, D, H, W]
        binary_mask  = raw_masks * binary_mask        # [bz, ch, D, H, W]
        ...
        with torch.no_grad():
                for i in range(ch):
                    for j in range (bz*D):
                        ...
                        if forground_num>30:
                            this_input_mask= torch.tensor( post_process_softmask(this_input_mask.cpu().detach().numpy()  ,this_input_image.cpu().detach().numpy()  ))
...
def CAM_to_slice_hardlabel(cam,video_predict):
        ...
        slice_hard_label = (count_masks>6)*1.0
